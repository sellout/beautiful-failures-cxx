...
in {
  ...
  ##
  ## TODO: For some reason, despite `stdenv` and `pkgs.llvmPackages` using LLVM
  ##       16, treefmt is using LLVM 15, and I haven’t had success in overriding
  ##       it. As a result, there are some settings here that are commented out
  ##       because they’re not supported by clang-format 15. Restore those once
  ##       LLVM 16 or later is used consistently.
  programs.treefmt.programs.clang-format.enable = true;
  project.file.".clang-format".text = lib.generators.toYAML {} {
    ## TODO: I’m not really sure how much my style aligns with any particular
    ##       style, but it might be good to fall back to one for the cases where
    ##       I don’t have a strong opinion, leaving this file clearer in its
    ##       intent.
    # BasedOnStyle = "LLVM";
    ...
    AlignTrailingComments = false; # "Never" with v16 # prevents spurious whitespace diffs
    ...
    # BreakAfterAttributes = "Never"; # added in v16 # uncertain, but compact
    BreakAfterJavaFieldAnnotations = false; # uncertain, but compact
    # BreakArrays = false; # added in v16 # only for JSON, but compact
    BreakBeforeBinaryOperators = "All"; # consistent
    BreakBeforeBraces = "Attach"; # compact & consistent
    BreakBeforeConceptDeclarations = true; # "Always" with v16 # seems redundant with AlwaysBreakTemplateDeclarations
    # BreakBeforeInlineASMColon = "OnlyMultiline"; # added in v16 # compact & consistent
    ...
    # NB: It’s important to format includes “inside out” to minimize the impact
    #     they can have on each other. See
    #     https://llvm.org/docs/CodingStandards.html#include-style for more.
    #
    #     However, here we only categorize the parts that can be identified
    #     syntactically. Any sub-ordering would be project-specific.
    IncludeCategories = [
    ...
    # IndentRequiresClause = true; # added in v16 # TODO: Don’t know why this doesn’t have an effect with `RequiresClausePosition: WithPreceding`
    IndentWidth = indentWidth;
    IndentWrappedFunctionNames = true; # consistent (but uncommon)
    # InsertBraces = true; # added in v16 # consistent, avoids accidentally breaking control flow when an additional statement is added
    # InsertNewlineAtEOF = true; # added in v16 # POSIX!
    InsertTrailingCommas = "Wrapped"; # consistent, prevents spurious whitespace diffs
    # IntegerLiteralSeparator = { # added in v16 # readability (although 8-digit groups may be too large)
    #   Binary = 8; # byte
    ...
    # RequiresClausePosition = "SingleLine"; # added in v16 # compact & consistent
